...
//        $dst_path = 'http://f4.topitme.com/4/15/11/1166351597fe111154l.jpg'; //网络图片
    ...
    $src_path  = 'http://'.$_SERVER['HTTP_HOST'].'/Public/'.$src_path; //二维码图片
...
//添加二维码至海报 $dst_path海报图片  $src_path二维码小图       $code 助力码用来做文件名
function createPoster($dst_path='' ,$src_path='' ,$code){
    /*给图片加文字水印的方法*/
    //路径要加上http://
//        $dst_path = 'http://f4.topitme.com/4/15/11/1166351597fe111154l.jpg'; //网络图片
    // $dst_path  = 'http://'.$_SERVER['HTTP_HOST'].'/Public/Uploads/qr/5b0a44e8b91d1.jpg';
...
//echo $dst_type;exit();
    ...
    switch($dst_type){
        case 1://GIF
            header("content-type:image/gif");
            imagegif($dst,'./Public/Uploads/posterImg/'.$code.'.jpg');
            break;
        case 2://JPG
            header("content-type:image/jpeg");
            imagejpeg($dst,'./Public/Uploads/posterImg/'.$code.'.jpg');
            break;
        case 3://PNG
            header("content-type:image/png");
            imagepng($dst,'./Public/Uploads/posterImg/'.$code.'.jpg');
            break;
        default:
            break;
        /*imagepng--以PNG格式将图像输出到浏览器或文件
